...
class Capture:
    ...
    def __enter__(self):
        self.process = subprocess.Popen(
            ["tcpdump", "-w", self.out_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def query(grid, queries):
    query_args = [arg for argtuple in zip(
        ["-T"]*len(queries), queries) for arg in argtuple]
    #print("executing", " ".join(["python3", "part1/client.py", "grid", str(grid)] + query_args + ["-t"]))
    subprocess.call(["python3", "/client/part1/client.py", "grid", str(grid)] +
                    query_args + ["-t"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
